...
# 4. Soft Exudates
# 5. Optic Disc
        # self.images_path_A = os.path.join(self.dataset_path, "ImagesA")
        # self.images_path_B = os.path.join(self.dataset_path, "ImagesB")
        # self.labels_path = os.path.join(self.dataset_path, "Labels")
    ...
    def __getitem__(self, index):
        ...
        # 加载标签图像
        jpg = Image.open(os.path.join(self.labels_path, name + ".jpg"))
        
        # 分别加载模态A和模态B的图像
        # jpg_A = Image.open(os.path.join(self.images_path_A, name + ".png"))
        # jpg_B = Image.open(os.path.join(self.images_path_B, name + ".png"))
        # # 加载标签图像
        # jpg = Image.open(os.path.join(self.labels_path, name + ".png"))

        # 对两种模态的图像以及标签图像进行数据增强
        jpg_A, jpg_B, jpg = self.get_random_data(jpg_A, jpg_B, jpg, self.input_shape, random=self.train)
        ...
        modify_jpg = np.zeros_like(jpg)
        modify_jpg[jpg <= 127.5] = 1
        seg_labels = modify_jpg
        seg_labels = np.eye(self.num_classes + 1)[seg_labels.reshape([-1])]
        seg_labels = seg_labels.reshape((int(self.input_shape[0]), int(self.input_shape[1]), self.num_classes + 1))

        # 返回模态A和模态B的图像以及标签
        return jpg_A, jpg_B, modify_jpg, seg_labels
    
    def rand(self, a=0, b=1):
        return np.random.rand() * (b - a) + a
    
    def get_random_data(self, image_A, image_B, label, input_shape, jitter=.3, hue=.1, sat=0.7, val=0.3, random=True):
...
def unet_dataset_collate(batch):
    ...
    jpgs = []
    seg_labels = []
    for img_A, img_B, jpg, labels in batch:
...
#         # 假设模态A和模态B的图像分别存储在以下两个文件夹中   先用单模态试试能不能运行，以后改一下文件夹名就行
#         self.images_path_A = os.path.join(self.dataset_path, "Images")
#         self.images_path_B = os.path.join(self.dataset_path, "Images")
#         self.labels_path = os.path.join(self.dataset_path, "Labels")
...
#         jpg_B = Image.open(os.path.join(self.images_path_B, name + ".png"))
#         # 加载标签图像
#         png = Image.open(os.path.join(self.labels_path, name + ".png"))

#         # 对两种模态的图像以及标签图像进行数据增强
#         jpg_A, jpg_B, png = self.get_random_data(jpg_A, jpg_B, png, self.input_shape, random=self.train)
...
#         # 返回模态A和模态B的图像以及标签
#         return jpg_A, jpg_B, modify_png, seg_labels
