...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="训练自定义的语义分割模型")
    ...
    #-----------------------------------------------------#
    # num_classes = 2
    parser.add_argument("--num_classes", type=int, default=2, help="需要的分类个数+1，默认为'2'")
    #-----------------------------------------------------#
    #   主干网络选择
    #   vgg
    #   resnet50
    ...
    #----------------------------------------------------------------------------------------------------------------------------#
    # pretrained  = True
    parser.add_argument("--pretrained", type=bool, default=True, help="是否使用主干网络的预训练权重，默认为'True'")
    #----------------------------------------------------------------------------------------------------------------------------#
    ...
    #----------------------------------------------------------------------------------------------------------------------------#
    # model_path  = ""
    parser.add_argument("--model_path", type=str, default="", help="模型的权值文件，默认为''")
    #-----------------------------------------------------#
    ...
    # Freeze_Epoch        = 0
    # Freeze_batch_size   = 2
    parser.add_argument("--Init_Epoch", type=int, default=0, help="模型当前开始的训练世代，默认为'0'")
    parser.add_argument("--Freeze_Epoch", type=int, default=0, help="模型在解冻后的batch_size，默认为'0'")
    parser.add_argument("--Freeze_batch_size", type=int, default=2, help="模型在解冻后的batch_size，默认为'2'")
    ...
    parser.add_argument("--UnFreeze_Epoch", type=int, default=50, help="模型总共训练的epoch，默认为'50'")
    parser.add_argument("--Unfreeze_batch_size", type=int, default=2, help="模型在解冻后的batch_size，默认为'2'")
    ...
    #------------------------------------------------------------------#
    # Freeze_Train        = False
    parser.add_argument("--Freeze_Train", type=bool, default=False, help="是否进行冻结训练，默认为'False'")
    ...
    #------------------------------------------------------------------#
    #save_period         = 5
    parser.add_argument("--save_period", type=int, default=5, help="保存模型权值的轮次数，默认为'5'")
    #------------------------------------------------------------------#
    ...
    #   数据集路径
    #------------------------------#
    # VOCdevkit_path  = 'Medical_Datasets/ABNORMAL'
    # VOCdevkit_path  = 'Medical_Datasets/DRIVE/train'
    # VOCdevkit_path  = 'Medical_Datasets/IDRiD'
    # VOCdevkit_path  = 'Medical_Datasets/CHASEDB1'
  
    parser.add_argument("--dataset", type=str, default='./dataset/CHASEDB1', help="数据集路径，默认为'./dataset/CHASEDB1'")
    # 解析命令行参数
    args = parser.parse_args()
    # 使用命令行参数
    VOCdevkit_path = args.dataset
...
#----------------------------------------------------------新多模态unet--------------------------------------------------------------------------#            
    ...
    if True:
        ...
        #-------------------------------------------------------------------#
        batch_size = Unfreeze_batch_size if Freeze_Train else Unfreeze_batch_size #不冻结试试
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            # r_image = detect_image(image, count=False, name_classes=name_classes)
            # r_image.show()
